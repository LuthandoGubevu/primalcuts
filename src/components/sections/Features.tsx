...
const featuresData: FeatureItem[] = [
  { name: 'Gluten Free', icon: WheatOff, description: 'Product is free from gluten.' },
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 md:gap-8 text-center">
          {featuresData.map((feat) => (
            <div key={feat.name} className="flex flex-col items-center space-y-3 group transform transition-transform duration-300 hover:scale-105">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-primary-foreground/10 border-2 border-primary-foreground/30 rounded-full flex items-center justify-center transition-all duration-300 group-hover:bg-accent group-hover:border-accent shadow-md group-hover:shadow-lg">
